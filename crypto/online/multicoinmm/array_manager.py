...
class ArrayManager(object):
    ...
    def __init__(self, name, pct_days, size=500):
        """Constructor"""
        self.count = 0
        self.name = name
        self.size = size
        self.inited = False
        self.pct_days = pct_days
        ...
        self.pct_1day_array = np.zeros(size) #前一个周期的pct
        self.pct_ln1day_array = np.zeros(size) #前一个周期的ln pct
        self.pct_ndays_array = np.zeros(size) #n日内的平均日收益率
        self.pct_lnndays_array = np.zeros(size) #n日内的平均日收益率的对数
        self.pctbodong_ndays_array = np.zeros(size) #n日内的日收益率的波动率
        self.pctbodong_lnndays_array = np.zeros(size) #n日内的日收益率的波动率
    ...
    def cur_pct_1day(self):
        return self.pct_1day_array[self.count]
    ...
    def cur_score(self):
        ...
        #return self.pct_lnndays_array[self.count]
        #return self.pct_lnndays_array[self.count] / self.pctbodong_lnndays_array[self.count]
    ...
    def update_bar(self, bar: BarData):
        ...
        #每日收益率
        self.pct_1day_array[self.count] = (bar.close_price - self.close_array[self.count - 1]) / \
                                           self.close_array[self.count - 1]
        self.pct_ln1day_array[self.count] = math.log(bar.close_price / self.close_array[self.count - 1])

        # N日收益率
        #if self.count == 5 and self.name in ('grtusdt'):
            #print(1)
        ...
        #self.pct_ndays_array[self.count] = self.pct_ndays_array[self.count] / stat_days

        #N日对数收益率
        self.pct_lnndays_array[self.count] = math.log((bar.close_price / self.close_array[self.count - stat_days]))


        #N日收益率波动
        self.pctbodong_ndays_array[self.count] = np.std(self.pct_ndays_array[self.count - stat_days + 1: (self.count)])
        #N日对数收益率波动
        self.pctbodong_lnndays_array[self.count] = np.std(self.pct_lnndays_array[self.count - stat_days + 1: (self.count)])
