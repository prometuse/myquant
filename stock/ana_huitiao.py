					...
					for s3_i in range(s2_end + TUPO_HUITIAO_MIN_GAP, min(s2_end + TUPO_HUITIAO_MIN_GAP + HUITIAO_STAT_MAX_RANGE, date_idx)):
						...
						if if_suolianghuitiao:
							...
							if mkt_value < 2000:
								...
								if cur_price_year_line_gap >= -0.05 and cur_price_year_line_gap <= 0.2:
									if_c5_ok = True
									DEBUG_INFO['c5'] = DEBUG_INFO['c5'] + 1

								#最高点下来，连续N天下跌
								# lianxu_down_days = 0
								# for k in range(highest_idx, date_idx + 1):
								# 	if pctChg[k] > 0.0:
								# 		lianxu_down_days = 0
								# 	if pctChg[k] < 0.0:
								# 		lianxu_down_days += 1
								# 	if lianxu_down_days >= MAX_LIANXU_MINUS:
								# 		if_c6_ok = False
								# 		break
								if_c6_ok = True

								#筑平台. 1.涨跌幅不能太大，2.有支撑，3.最近 量价齐升
								# RECENT_STAT_DAYS = 5
								# recent_pre_vol = np.mean([volume[x] for x in range(date_idx - 8 + 1, date_idx - RECENT_STAT_DAYS + 1)])
								# if cur_vol / recent_pre_vol > 1.3:
								# 	if_c7_ok = True
								if_c7_ok = True

								# rise_days = 0
								# for k in range(date_idx - RECENT_STAT_DAYS + 1, date_idx + 1):
								# 	if pctChg[k] > 0.0:
								# 		rise_days += 1
								# if rise_days >= (RECENT_STAT_DAYS - 2):
								# 	if_c8_ok = True

								#最近有一次涨停
								for k in range(date_idx - 3, date_idx + 1):
									if pctChg[k] >= 9.9:
										if_c8_ok = True
										break
								...
								if IF_DEBUG:
									...
									print('c3: {}'.format(if_c3_ok))
									print('c4: {}'.format(if_c4_ok))
									print('c5: {}'.format(if_c5_ok))
									print('c6: {}'.format(if_c6_ok))
									print('c7: {}'.format(if_c7_ok))
									print('c8: {}'.format(if_c8_ok))
...
def ana_days(mode, *args):
	...
	target_begin_date = '2020-12-23'
	target_begin_idx = date_list.index(target_begin_date)

	target_end_date = '2020-12-25'
