				...
				if not if_tiqianhuitiao:
					...
					#DEBUG if date_list[i] == '2020-07-06':
						...
						if if_suolianghuitiao:
							...
							cur_vol = np.mean([volume[j] for j in range(date_idx - 3, date_idx)])
							...
							if mkt_value < 500:
								...
								#最近 量价齐升
								RECENT_STAT_DAYS = 3
								recent_pre_vol = np.mean([volume[j] for j in range(, date_idx - RECENT_STAT_DAYS + 1 )])
